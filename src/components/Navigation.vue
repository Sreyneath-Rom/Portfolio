<template>
  <header class="fixed top-0 left-0 right-0 z-50" :class="headerClasses">
    ...
    <div
      class="absolute inset-0 -z-10 bg-white/10 dark:bg-black/30 backdrop-blur-2xl border-b border-white/20"
    ></div>
    ...
    <nav class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
      ...
      >
        <div class="relative">
          <!-- Pulsing glow ring -->
          <div
            class="absolute -inset-3 rounded-full bg-gradient-to-r from-cyan-400 via-purple-500 to-pink-500 opacity-70 blur-2xl group-hover:opacity-100 animate-pulse transition-all duration-700"
          ></div>

          <div class="flex items-center gap-3">
            <div
              class="w-10 h-10 rounded-full overflow-hidden shadow-lg"
              @mousemove.stop
            >
              <img
        ...
        <span
          class="text-3xl font-black tracking-tighter bg-gradient-to-r from-cyan-400 via-purple-400 to-pink-400 bg-clip-text text-transparent"
        >
          SREYNEATH ROM
        </span>
      </router-link>
      ...
      <div class="hidden lg:flex items-center gap-6">
        ...
        >
          <span class="relative z-10 flex items-center gap-3 text-white">
            <span
              class="material-symbols-outlined text-2xl group-hover:scale-125 transition-transform"
              >mail</span
            >
            Hire Me
          </span>
          <span
            class="absolute inset-0 -translate-x-full bg-white/40 skew-x-12 transition-transform duration-1000 group-hover:translate-x-full"
          ></span>
        </a>
      </div>
        ...
        >
          ...
          >
            <div class="flex flex-col h-full">
              <div
                class="p-8 border-b border-white/10 flex items-center justify-between"
              >
                <div class="flex items-center gap-5">
                  <div
                    class="p-4 rounded-full bg-gradient-to-br from-cyan-500 to-purple-600 shadow-2xl"
                  >
                    <span class="material-symbols-outlined text-4xl text-white"
                      >person</span
                    >
                ...
                <button
                  @click="showMobileMenu = false"
                  class="p-3 rounded-full bg-white/10 hover:bg-white/20"
                >
                  <span class="material-symbols-outlined text-4xl text-white"
                    >close</span
                  >
...
const headerClasses =
  "transition-all duration-500 supports-backdrop-blur:bg-white/10 dark:supports-backdrop-blur:bg-black/30";
</script>
